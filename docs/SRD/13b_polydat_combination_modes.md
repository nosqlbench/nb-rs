...
- One `PolydatProgram`, one `PolydatState`.
...
domain-specific binding macros, and most `.polydat` files are inlined.
...
See SRD 13 §"Module Inlining" and `polydat/src/dsl/modules.rs`.
...
The inner kernel is a real `PolydatProgram` + `PolydatState` whose `extern`
...
- "embedding multiple `.polydat` files into a single program" — that's
...
- **Scope** is structural and static: a `PolydatProgram` + `PolydatState`
