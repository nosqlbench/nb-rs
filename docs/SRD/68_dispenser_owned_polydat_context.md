# SRD-68 — Dispenser-Owned Polydat Context and Single-Surface Resolution
...
**Owner:** nmbrs-runtime (synthesis, executor, scope, dispenser
construction), nmbrs-adapter-* (every adapter's `map_op`).
...
(op execution pipeline), SRD-67 (parent-gated Polydat sub-context
...
How an op dispenser owns its Polydat context, how that context relates to
the dispenser's enclosing scope and to per-fiber state, and how
op-template bind points get resolved at cycle time.

SRD-67 says *how* a child Polydat kernel comes into existence as a function
...
materialises a child Polydat context (or reuses the parent's, when no
...
A dispenser has **exactly one** Polydat context for op-template-scope
...
`PolydatKernel` internals — no `program()`, no `state()`, no
`scope_coordinates()`, no metadata APIs. The trait is the wall
between adapter code and Polydat runtime mechanics; what the dispenser
...
instance. The fiber holds these as `Vec<PolydatKernel>` indexed parallel
...
adapter.map_op(template)
  ...
  — has no Polydat context
  ↓
OpBuilder synthesis:
  op_template_kernels: HashMap<String, PolydatKernel>
...
adapter.map_op(template, &parent_subcontext_builder)
  — assembles its op-template Polydat matter
...
fiber spawn:
  ...
  holds Vec<PolydatKernel> indexed parallel to dispensers
  ↓
cycle execution:
...
- `OpBuilder::op_template_kernels: HashMap<String, PolydatKernel>`
...
/// its bound Polydat context. The wall between adapter code and GK
...
`PolydatKernel` implements `WireSource`. The implementation pulls
...
Dispensers store nothing more than `Arc<PolydatKernel>` (the canonical
...
that need no Polydat matter of their own simply ignore it (the
...
- **Fiber spawn order:** fibers spawn after all dispensers exist.
  A fiber's `Vec<PolydatKernel>` is built in one pass over the
...
- Define `WireSource` on `nmbrs-runtime::adapter`.
...
- Tests: existing suites pass unchanged; one new test covers
  `WireSource` round-trip on `PolydatKernel`.
...
- Fiber spawn walks dispenser registry, calls `build_subscope` on
  each dispenser's canonical kernel, stores `Vec<PolydatKernel>`
...
5d. **LUT delete** — drop `OpBuilder::op_template_kernels` HashMap,
    ...
    per-fiber `per_op_kernels: Vec<Option<PolydatKernel>>` and Push 5b's
