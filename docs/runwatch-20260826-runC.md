...
### 17:56 UTC — t+14h08m — deadlock holds (39 min); ingest still flowing into memtables; flush cliff pending

- **Wedge unchanged**: last merge progress line remains 17:17:40 (123,940/123,941); zero phase transitions since. Device 1.5k r/s / 29% util — pool still parked.
- **Client alive and ingesting**: MutationStage active=5, 120.0M mutations completed; the on-heap memtable index path bypasses the dead pool, so writes are still accepted. tpstats shows **no flush in flight and none pending** (MemtableFlushWriter 0/0, 231 completed) — flushes stopped being requested around the wedge; the accumulating memtable is the fuse. When the next flush fires, its SAI segment build will queue into the dead pool/permits and hang; MemtablePostFlush will then back up → MutationStage blocks → client windowed saturation breaker (first live test) should abort the load.
- Cgroup: anon 106.8G / file 35.9G, max=0. Table: see below — SSTable count static since the wedge.
- Gate: 0 cluster-cost lines; 0 assertions (deadlock is Cassandra-side concurrency, not a mapping fault).
- Age forensics (this hour): the deadlock topology is **committed code, 4 weeks old** (2026-07-28 5280270861 passes `compactionBuildPool()` into `refine` inside `trainingLock.writeLock()`; insert fan-out on the same pool since 2026-07-08 243a05f588); `trainingLock` itself 19 months old (#1471); today's `BoundedParallelExecutor` (00:43, jar 01:06, maiden run) supplied the wait mechanics and 13.7h of saturation supplied the odds. HEAD is implicated, not just the 29-file WIP.
- Trend: stable wedge with a burning fuse — decision (restart vs fix-first) still open with the user; the 63.6M's five-window 3.9±0.3 min/M base-layer result stands regardless.

> **17:59 correction to the 17:56 entry**: flushes are NOT stopped — debug.log shows completed flushes through ~17:36 and SSTables grew 28→31 (703.1 GB) post-wedge. Data flushes complete with their SAI segment builds *deferred* (the acquireBuildPermit queue — 5 waiters — and the pending "Secondary index build" rows are the accumulating debt). So the fuse is slower than stated: commitlog/mutations keep draining; what accumulates is unindexed-segment debt and permit waiters, with the hard stop arriving when flush threads themselves join the permit/pool wait. "SSTable count static since the wedge" was wrong.

### 18:33 UTC — RUN C ENDED — operator broke the deadlock (SIGKILL 18:10:33), then wiped: `data/baselines/` gone, fresh node up 18:17:31

- Sequence (journalctl + system.log): SIGKILL 18:10:33 → intermediate start ~18:11 (28,091 system-table mutations later replayed) → wipe (no `baselines` data dir; old commitlog gone) → final start 18:17:31, CQL listening 18:17:42. The 63.6M merge is forfeit — frozen at 99.9992% for 53 min, never printed its wall verdict. **Run C's scientific record closes with the five-window base-layer result: 3.9±0.3 min/M through 99.99% of a 63.6M symmetric merge, gate clean throughout.**
- **Client post-wipe (BREAKER DEFECT, live specimen)**: loader fails 100% of attempts (~54/s; attempt_failure 48,518→49,985 in 27 s, +1,467 = +100% of attempts; zero successes since 18:17; rows frozen at 20.58M). Windowed clause inputs are richly satisfied in the metrics DB (recent_attempt_total ≈3k ≫ floor 20 @ ratio ~1.0; terminal ≈148/55s ≥ floor 100) — **and no abort fires**. The `recent_*` values reach stop_when via daemon-evaluated metricsql → shared wires (yaml 1128-41); the published wire gauges render with NO value (NaN/empty suspicion) → defect is in the daemon→wire→condition chain, not thresholds. Console `ok:100% att:100%` are lifetime ratios over 20.6M — 26k new failures move att 99.88%→99.76%, still rounds to 100; `e:`/`r:` are lifetime absolutes. Constraint recorded: breakers must remain client-side only (driver-visible signals).
- Monitoring teardown: cron defbf253 deleted, Monitor bue33p87x stopped (run over).

> **19:05 root cause of the breaker no-fire (closes the 18:33 defect)** — nbrs-level, not polydat, not the metricsql engine. The daemon bindings embed op-template curly syntax inside a polydat binding string: `metricsql_scalar("...{part=\"{part}\"...}")`. Bindings do NOT template-interpolate — params are constants there (model.rs: "bind points in op templates and as constants in Polydat bindings") — so the engine received the literal selector `part="{part}"`, matched nothing (verified: `nbrs metrics query` with literal `{part}` → "(no series matched)"; with the real label → 1,388), `metricsql_scalar` projected empty→NaN, the wires held NaN from birth (every stored sample NULL back to series creation), and every stop_when comparison on NaN was false. The 2026-08-25 CI example fired because its selectors were LITERAL — the exact untested delta. Exonerating evidence: servo wires (CQL-derived, no metricsql) carry real values through the identical shared-cell+publish machinery. Fixes: (1) yaml — phase-only selectors (parts run sequentially; matches the proven example) or build the selector via string concat from the `part` constant; (2) nbrs hardening — warn on scalar-over-empty, render NaN wires distinctly from "no data", make a NaN-poisoned stop_when loud. Yesterday's "phase-scoping" fix replaced a diluted-but-live signal (0.50) with a dead one (NaN) — the example validated the clause logic but not the interpolation.
