...
| **Dependencies flow strictly downward; every subsystem has a declared contract surface and its edges are CI-enforced.** The workspace is an 8-layer DAG (L0 leaves → L7 binary); `[dependencies]` edges only point to a strictly lower layer. polydat depends only on `polydat-derive` (it stays independently extractable). Adapters never depend on adapters (except `testkit→stdout`). No consumer reaches past a crate's declared public surface (e.g. polydat's deep internals). **polydat is self-contained — its docs and source never reference the nbrs `docs/SRD/` layer (D7), so it stays extractable.** Every subsystem is held to the five-pillar Subsystem Treatment Standard. | [SRD 05](SRD/05_dependency_rules.md), [SRD 00b](SRD/00b_subsystem_standard.md), gate: `nbrs/tests/architecture_rules.rs` |
