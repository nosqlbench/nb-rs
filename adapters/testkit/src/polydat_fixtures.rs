...
//! binary for them to appear in the Polydat function registry.

use std::collections::HashMap;
use std::sync::Mutex;

use polydat::ast::{PolydatNode, NodeMeta, Port, Slot, SlotType, Value};
...
impl PolydatNode for SideEffectSequenceNextCycling {
    fn meta(&self) -> &NodeMeta { &self.meta }
...
impl PolydatNode for SideEffectSequenceNextNoncycling {
    fn meta(&self) -> &NodeMeta { &self.meta }
    fn eval(&self, _inputs: &[Value], outputs: &mut [Value]) {
        outputs[0] = Value::U64(self.value);
    }
...
impl PolydatNode for SideEffectSequenceReset {
    fn meta(&self) -> &NodeMeta { &self.meta }
    fn eval(&self, _inputs: &[Value], outputs: &mut [Value]) {
    ...
    match name {
        ...
        "side_effect_sequence_next_cycling" => {
            ...
            Some(SideEffectSequenceNextCycling::new(path, csv)
                .map(|n| Box::new(n) as Box<dyn PolydatNode>))
        }
...
mod tests {
    ...
    fn cycling_caches_within_a_session() {
        // The Polydat assembly path constructs the node multiple times
