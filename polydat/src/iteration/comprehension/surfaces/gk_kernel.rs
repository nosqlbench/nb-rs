...
//! and polydat's [`GkKernel`] scope-binding primitives.
//!
//! Implements `KernelScope` for a `GkKernelScope` wrapper that
//! holds a `(canonical, parent)` `GkKernel` pair. Each
//! `scope(coords)` call delegates to
//! [`GkKernel::for_iteration`] — the established polydat
...
//! once `KernelScope for GkKernelScope` exists, the algebra-
//! layer [`ScopedKernelStream<GkKernelScope>`] can replace
...
/// Wrapper around a `(canonical, parent)` `GkKernel` pair that
...
/// `GkKernel::for_iteration(&canonical, &parent, &bindings)`.
#[derive(Debug, Clone)]
pub struct GkKernelScope {
...
impl GkKernelScope {
    ...
    pub fn new(canonical: Arc<GkKernel>, parent: Arc<GkKernel>) -> Self {
    ...
    pub fn canonical(&self) -> &Arc<GkKernel> {
        &self.canonical
    }

    /// Access the parent kernel.
    pub fn parent(&self) -> &Arc<GkKernel> {
        &self.parent
    }
}

impl KernelScope for GkKernelScope {
    /// Each scope produces a fresh `Arc<GkKernel>` — the
    /// per-iteration child kernel. Consumers can clone the Arc
    /// cheaply if they need multiple references.
    type Scoped = Arc<GkKernel>;

    fn scope(&self, coords: &Tuple) -> Arc<GkKernel> {
        let bindings: Vec<(String, Value)> = coords
            .bindings
            .iter()
            .map(|(name, val)| (name.clone(), tuple_value_to_polydat_value(val)))
            .collect();
        GkKernel::for_iteration(&self.canonical, &self.parent, &bindings)
    }
}
