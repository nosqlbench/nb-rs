...
/// Base-2 radical inverse. Same building block as Halton's
/// base-2 axis; Sobol differs by per-axis direction numbers
/// for higher K but the simple form here uses identity
/// directions.
...
/// K-D Sobol point at index `i`. With identity direction
/// numbers per axis (the simple form), this is the base-2
/// van-der-Corput sequence on every axis — which produces
/// degenerate behavior for K > 1 (all axes the same value).
/// To avoid that for testing, we offset each axis by a per-axis
/// gray-code shift so the K-D point set is at least non-collapsed.
fn sobol_point(i: u64, dim: usize) -> Vec<f64> {
    (0..dim)
        .map(|k| {
            // Axis k uses i shifted by a per-axis offset so the
            // sequences don't all collapse to the same value.
            // This is NOT production-quality Sobol; production
            // requires Joe-Kuo direction numbers per axis. The
            // offset preserves determinism and avoids the
            // collapse.
...
mod tests {
    ...
    #[test]
    fn deterministic_indexed_apply() {
        let idx = IndexFn::Lattice { axis_sizes: vec![20, 20] };
        let a = Sobol.indexed_apply(&idx, Some(10));
        let b = Sobol.indexed_apply(&idx, Some(10));
        assert_eq!(a, b);
    }

    #[test]
    fn indexed_apply_produces_unique_discrete() {
        let idx = IndexFn::Lattice { axis_sizes: vec![50, 50] };
        let out = Sobol.indexed_apply(&idx, Some(20));
    ...
    fn continuous_box_draws() {
        ...
        let out = Sobol.indexed_apply(&idx, Some(50));
