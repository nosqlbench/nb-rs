...
mod tests {
    ...
    fn build_err(wire_count: usize) -> String {
        let wires: Vec<crate::compile::assembly::WireRef> = (0..wire_count)
            .map(|i| crate::compile::assembly::WireRef::input(format!("w{i}")))
            .collect();
        let wire_types: Vec<crate::ast::PortType> = wires.iter()
            .map(|_| crate::ast::PortType::U64).collect();
