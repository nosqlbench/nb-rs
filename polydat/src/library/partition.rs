...
fn expect_partition<'a>(value: &'a Value, fn_name: &str) -> &'a crate::iteration::cursor_partition::Partition {
...
impl PolydatNode for PartitionsOf {
    ...
    fn eval(&self, inputs: &[Value], outputs: &mut [Value]) {
        ...
        let spec = crate::iteration::cursor_partition::parse(&spec_str)
            .unwrap_or_else(|e| panic!("partitions: bad spec `{spec_str}`: {e}"));
        let parts = crate::iteration::cursor_partition::resolve(&spec, 0, self.extent)
...
mod tests {
    use super::*;
    use crate::iteration::cursor_partition::Partition;
