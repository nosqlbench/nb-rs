...
//!    in-test mirror of [`nbrs_variates::assembly::auto_adapter`]. If the
...
use nbrs_variates::dsl::compile::{compile_gk, compile_gk_with_log};
use nbrs_variates::dsl::events::{CompileEvent, CompileEventLog};
use nbrs_variates::dsl::registry::{self, FuncSig};
use nbrs_variates::node::{PortType, SlotType};

// ─── Expected-adapter table ───────────────────────────────────────
//
// Mirrors `nbrs_variates::assembly::auto_adapter`. When the compiler's
...
fn generate_module(rng: &mut Rng, sigs: &[FuncSig], n_bindings: usize) -> String {
    ...
    for i in 0..n_bindings {
        ...
        let materialize = |rng: &mut Rng, p: &nbrs_variates::dsl::registry::ParamSpec, defined: &[String]| -> String {
            match p.slot_type {
...
fn strict_values_inserts_nonzero_assertion_on_mod_wire() {
    use nbrs_variates::dsl::events::CompileEvent;
