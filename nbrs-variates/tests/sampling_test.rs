...
use nbrs_variates::nodes::arithmetic::{AddU64, Interleave, MixedRadix, ModU64};
use nbrs_variates::nodes::hash::Hash64;
use nbrs_variates::sampling::alias::AliasSample;
use nbrs_variates::sampling::icd::{ClampF64, IcdSample, UnitInterval};
...
fn build_normal_pipeline() -> nbrs_variates::kernel::GkKernel {
...
fn build_correlated_pipeline() -> nbrs_variates::kernel::GkKernel {
    let mut asm = GkAssembler::new(vec!["cycle".into()]);
    asm.add_node("seed", Box::new(Hash64::new()), vec![WireRef::input("cycle")]);
    asm.add_node("quantile", Box::new(UnitInterval::new()), vec![WireRef::node("seed")]);
...
fn build_independent_pipeline() -> nbrs_variates::kernel::GkKernel {
    let mut asm = GkAssembler::new(vec!["cycle".into()]);
...
fn build_weighted_entity_pipeline() -> nbrs_variates::kernel::GkKernel {
    let mut asm = GkAssembler::new(vec!["cycle".into()]);
...
fn build_sensor_workload() -> nbrs_variates::kernel::GkKernel {
    let mut asm = GkAssembler::new(vec!["cycle".into()]);
