...
fn extern_usable_as_wire_argument() {
    ...
    kernel.state().set_input(idx, nbrs_variates::node::Value::U64(100));
    kernel.set_inputs(&[42]);
    let v1 = kernel.pull("result").as_u64();

    // Different extern value should produce different hash
    kernel.state().set_input(idx, nbrs_variates::node::Value::U64(200));
    kernel.set_inputs(&[42]);
...
fn extern_in_arithmetic_expression() {
    ...
    kernel.state().set_input(idx, nbrs_variates::node::Value::U64(10));
    kernel.set_inputs(&[5]);
...
fn extern_input_usable_as_output() {
    ...
    kernel.state().set_input(idx, nbrs_variates::node::Value::U64(128));
    kernel.set_inputs(&[0]);
...
fn eval_const_integer_arithmetic() {
    let v = nbrs_variates::dsl::compile::eval_const_expr("4 * 4").unwrap();
    assert_eq!(v.as_u64(), 16);
}

#[test]
fn eval_const_float_arithmetic() {
    let v = nbrs_variates::dsl::compile::eval_const_expr("3.0 + 0.14").unwrap();
    assert!((v.as_f64() - 3.14).abs() < 0.001);
}

#[test]
fn eval_const_rejects_cycle_dependent() {
    let result = nbrs_variates::dsl::compile::eval_const_expr("hash(cycle)");
    assert!(result.is_err(), "cycle-dependent expr should not be const");
}

#[test]
fn eval_const_nested() {
    let v = nbrs_variates::dsl::compile::eval_const_expr("(2 + 3) * (4 + 1)").unwrap();
