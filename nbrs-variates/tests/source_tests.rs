...
use nbrs_variates::source::{DataSourceFactory, RangeSourceFactory, SourceItem};
...
fn cursor_keyword_lexes() {
    let tokens = nbrs_variates::dsl::lexer::lex("cursor base = range(0, 100)").unwrap();
    assert!(matches!(tokens[0].kind, nbrs_variates::dsl::lexer::TokenKind::Cursor));
}

#[test]
fn dot_token_lexes() {
    let tokens = nbrs_variates::dsl::lexer::lex("base.ordinal").unwrap();
    assert!(matches!(tokens[0].kind, nbrs_variates::dsl::lexer::TokenKind::Ident(_)));
    assert!(matches!(tokens[1].kind, nbrs_variates::dsl::lexer::TokenKind::Dot));
    assert!(matches!(tokens[2].kind, nbrs_variates::dsl::lexer::TokenKind::Ident(_)));
}

#[test]
fn dot_in_float_not_confused() {
    // 3.14 should lex as a float, not ident.dot.ident
    let tokens = nbrs_variates::dsl::lexer::lex("x := 3.14").unwrap();
    assert!(matches!(tokens[2].kind, nbrs_variates::dsl::lexer::TokenKind::FloatLit(f) if (f - 3.14).abs() < 0.001));
}

#[test]
fn cursor_decl_parses() {
    let tokens = nbrs_variates::dsl::lexer::lex(
        "cursor base = range(0, 100)\nid := hash(cycle)"
    ).unwrap();
    let ast = nbrs_variates::dsl::parser::parse(tokens).unwrap();
    assert!(matches!(ast.statements[0], nbrs_variates::dsl::ast::Statement::Cursor(_)));
}
...
fn cursor_keyword_as_binding_name_error() {
    ...
    let tokens = nbrs_variates::dsl::lexer::lex("cursor := hash(cycle)");
    ...
    let result = nbrs_variates::dsl::parser::parse(tokens.unwrap());
...
fn dot_access_without_source_name() {
    ...
    let result = nbrs_variates::dsl::lexer::lex(".ordinal");
...
fn multiple_cursor_declarations() {
    let src = r#"
        ...
        id := hash(cycle)
    "#;
    let tokens = nbrs_variates::dsl::lexer::lex(src).unwrap();
    let ast = nbrs_variates::dsl::parser::parse(tokens).unwrap();
    let source_count = ast.statements.iter()
        .filter(|s| matches!(s, nbrs_variates::dsl::ast::Statement::Cursor(_)))
        .count();
...
fn chained_dot_access_rejected() {
    ...
    let tokens = nbrs_variates::dsl::lexer::lex("x := base.ordinal.extra").unwrap();
    // The parser should parse base.ordinal as FieldAccess, then .extra
    // is unexpected in expression position
    let ast = nbrs_variates::dsl::parser::parse(tokens);
...
fn advancer_targets_correct_cursors() {
    use nbrs_variates::source::Cursors;
    ...
    let mut factories: HashMap<String, Arc<dyn nbrs_variates::source::DataSourceFactory>> = HashMap::new();
...
fn advancer_empty_when_no_sources_referenced() {
    ...
    let factories: HashMap<String, Arc<dyn nbrs_variates::source::DataSourceFactory>> = HashMap::new();
    let advancer = Cursors::for_fields(program, &["id"], &factories);
...
fn limit_node_compiles_and_clamps_extent() {
    use nbrs_variates::dsl::compile::compile_gk_with_libs_and_limit;
