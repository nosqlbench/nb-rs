...
/// Futures that complete with a normal result and the
/// server-side trace UUID. The UUID is `Some` when the
/// statement had `set_tracing(true)` set before execute (the
/// driver returns `CASS_ERROR_LIB_NO_TRACING_ID` when tracing
/// wasn't enabled, which we surface as `None`). Pair this with
/// [`crate::Session::execute_with_tracing`] to retrieve the
/// `system_traces.sessions` partition key for an executed
/// query.
impl Completable for (CassResult, Option<Uuid>) {
    unsafe fn get(_session: Session, inner: *mut _Future) -> Option<Self> {
        let mut raw: _Uuid = std::mem::zeroed();
        let trace_id = if cass_future_tracing_id(inner, &mut raw) == CASS_OK {
            Some(Uuid::build(raw))
        } else {
            None
        };
        cass_future_get_result(inner)
            .as_ref()
            .map(|r| (CassResult::build(r as *const _), trace_id))
    }
}
