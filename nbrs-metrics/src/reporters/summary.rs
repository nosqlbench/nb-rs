...
/// Compute one aggregate row per distinct value-tuple across
/// `group_by` keys. The label column reads
/// `**mean(recall) [k=10, optimize_for=RECALL]**` so the user
/// can identify which group each row corresponds to.
#[cfg(feature = "sqlite")]
