...
//!
//! Placed in `nbrs-metrics` (layer 2, per SRD-05) so both `nbrs-metrics` (the
//! scheduler) and `nbrs-runtime` (which sizes the `workers` runtime) can read
//! it without an upward dependency edge.
//!
//! Config resolves from env + core count with reasonable defaults (SRD-102 §4);
//! CLI-flag overlay lives in the `nbrs` binary. Unknown pool/policy names are
...
/// Raw (unparsed) CLI `--threads.*` override values, collected by the `nbrs`
...
/// identical to the `NBRS_THREADS_*` env path. Each field mirrors one env var:
/// `timing`↔`NBRS_THREADS_TIMING`, `io`↔`NBRS_THREADS_IO`,
/// `workers`↔`NBRS_THREADS_WORKERS`, `timing_sched`↔`NBRS_THREADS_TIMING_SCHED`,
/// `timing_pin`↔`NBRS_THREADS_TIMING_PIN`.
...
pub struct ThreadPoolConfig {
    ...
    /// builder in `nbrs-runtime`; the registry does not own that runtime).
    pub workers: usize,
}
...
impl ThreadPoolConfig {
    ...
    /// Overlay `NBRS_THREADS_*` env vars onto the defaults. Returns `Err` on a
    /// malformed value (never silently ignored). The `nbrs` binary layers CLI
    ...
    /// lives here (not in the `nbrs` binary) so a malformed CLI value is the
...
mod tests {
    ...
    fn cli_override_absent_leaves_resolved_value() {
        ...
        // since no NBRS_THREADS_* is set in the unit-test env).
