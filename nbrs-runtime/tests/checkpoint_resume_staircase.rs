...
use nbrs_runtime::checkpoint::{Checkpoint, PhaseStatus, storage};
...
fn run_args(args: &[String]) {
    ...
    rt.block_on(async {
        ...
        let _ = nbrs_runtime::runner::run(args).await;
    });
}
...
/// (`nbrs-runtime/`), so we walk up one level.
fn workspace_path(rel: &str) -> PathBuf {
    let mut p = std::env::current_dir().expect("cwd");
    if p.ends_with("nbrs-runtime") {
        p.pop();
    }
    p.join(rel)
}
