...
use nbrs_runtime::fixture::ScopeFixture;
use nbrs_runtime::validation::ValidatingDispenser;
use nbrs_runtime::wrappers::{ConditionalDispenser, DelayDispenser, WhileWrapper};
...
fn conditional_unknown_name_errors_loud() {
    ...
    let inner: Arc<dyn OpDispenser> = Arc::new(NoopDispenser);
    let labels = Labels::of("session", "test");
    let metrics = Arc::new(nbrs_runtime::activity::ActivityMetrics::new(&labels));
...
fn delay_unknown_name_errors_loud() {
    ...
    let mut fx = ScopeFixture::new(program.clone());
    let inner: Arc<dyn OpDispenser> = Arc::new(NoopDispenser);
    let result = DelayDispenser::wrap(
...
fn duplicate_registration_is_idempotent_across_consumers() {
    ...
    let _delayed = DelayDispenser::wrap(
