...
//! `nbrs session` — session lifecycle management.
//!
//! `nbrs session init` creates a new, empty session (its directory,
//! `metrics.db` schema, and the invariant `session` metadata) WITHOUT
//! running any workload. A later `nbrs run` / `nbrs refine` attaches
...
fn handle_bare(_p: ParsedCommand) -> Result<(), String> {
    eprintln!(
        "usage: nbrs session init [name=<name> | template=<template>] \
         ...
         e.g.  nbrs session init template=bench_{{date}}_{{time}}"
    );
    Ok(())
}
