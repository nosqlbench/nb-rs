...
//! `nbrs blueprint` — authoring support for the SRD-108/109
...
use nbrs_workload::model::{ParsedOp, Workload};
...
pub fn blueprint_command(args: &[String]) {
    match args.first().map(String::as_str) {
        ...
        _ => {
            eprintln!("nbrs blueprint <subcommand>");
            eprintln!("  list                           List blueprints (workloads with abstract op slots)");
            eprintln!("  template <blueprint> [<file>]  Generate an implementation skeleton (stdout, or <file>)");
            eprintln!();
...
fn load_blueprint(name: &str) -> Result<(String, Workload), String> {
    ...
    let (identity, merged) = if let Some(b) = catalog::lookup(name) {
        (b.name.to_string(), nbrs_workload::extends::load_and_merge_bundled(b)?)
        ...
        return Err(format!(
            "`{name}` is neither a bundled workload nor a local file — \
             `nbrs blueprint list` names the bundled blueprints"));
    };
...
fn list_blueprints() -> Result<(), String> {
    ...
    for entry in nbrs_workload::catalog::iter() {
        ...
        let Ok((identity, workload)) = load_blueprint(entry.name) else { continue };
        let slots = nbrs_workload::implements::unbound_abstract_slots(&workload);
        if slots.is_empty() {
            continue;
        }
        let desc = workload.description.as_deref()
    ...
    println!();
    println!("scaffold one: nbrs blueprint template <name> [<out.yaml>]");
    Ok(())
}
...
fn template(args: &[String]) -> Result<(), String> {
    let Some(name) = args.first() else {
        return Err("usage: nbrs blueprint template <blueprint> [<out.yaml>]".into());
    };
    ...
    match args.get(1) {
        ...
        Some(out) => {
            ...
            println!("wrote {out} — implementation skeleton for `{identity}` \
                      ({} slot(s); fill the TODOs)", slots.len());
        }
    }
    Ok(())
}
...
fn generate_template(identity: &str, workload: &Workload) -> String {
    ...
    let _ = writeln!(out, "#!/usr/bin/env nbrs");
    let _ = writeln!(out, "# Implementation skeleton for the `{identity}` blueprint,");
    let _ = writeln!(out, "# generated by `nbrs blueprint template {identity}`.");
    let _ = writeln!(out, "#");
    let _ = writeln!(out, "# Fill each TODO with your protocol's LITERAL request form.");
    let _ = writeln!(out, "# An implementation phase carries ONLY `ops:` — scaffolding");
    let _ = writeln!(out, "# (cycles, concurrency, governance, metrics, evaluations)");
    let _ = writeln!(out, "# stays in the blueprint, and redeclaring it here is a load");
    let _ = writeln!(out, "# error. Interface types are proven at synthesis; this file");
    let _ = writeln!(out, "# already BINDS as-is (every slot covered, every promised");
    let _ = writeln!(out, "# wire declared).");
    let _ = writeln!(out, "#");
    let _ = writeln!(out, "#   nbrs run workload=<this-file> [scenario=…] [param=value …]");
    let _ = writeln!(out);
    ...
    if !workload.declared_params.is_empty() {
        let _ = writeln!(out, "# The blueprint already declares these params (reference");
    ...
    let _ = writeln!(out, "  # TODO: protocol params (endpoints, credentials, knobs).");
    let _ = writeln!(out);
    let _ = writeln!(out, "phases:");

    for phase_name in &workload.phase_order {
        let Some(phase) = workload.phases.get(phase_name) else { continue };
        let abstract_ops: Vec<&ParsedOp> = phase.ops.iter()
        ...
        for op in abstract_ops {
            ...
            let iface = op.abstract_interface.as_ref()
                .expect("filtered to abstract ops");
            if iface.needs.is_empty() && iface.yields.is_empty()
                && iface.results.is_empty()
            {
                let _ = writeln!(out, "        # Free-form slot — no typed interface; any op body.");
            }
            if !iface.needs.is_empty() {
                let _ = writeln!(out, "        # needs — wires the blueprint GUARANTEES; reference");
                let _ = writeln!(out, "        # them as {{name}} in the op body:");
                for (n, t) in &iface.needs {
                    let _ = writeln!(out, "        #   {n}: {t}");
                }
            }
            let _ = writeln!(out, "        stmt: \"TODO: {phase_name}.{} request\"", op.name);
            if !iface.yields.is_empty() {
                let _ = writeln!(out, "        # yields — wires you MUST deliver; point each capture");
            ...
            if !iface.results.is_empty() {
                let _ = writeln!(out, "        # results — projected wires you MUST deliver; give each");
                let _ = writeln!(out, "        # a path expression over the response body ([*] projects");
