...
//! pinned to `nbrs_workload::vocab` (the enumerable registry the
...
/// (`nbrs_workload::vocab`) so additions there are seen here.
fn vocab_universe() -> BTreeSet<String> {
    use nbrs_workload::vocab as v;
...
fn run_one(sandbox: &Sandbox, yaml: &str, idx: usize) -> Outcome {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_nbrs"))
