...
//! Smoke test for the `nbrs check` subcommand — the subprocess
//! verification path (`verify_target` → `verify_path` → `run_case`),
//! which re-spawns `nbrs run` per case and checks its combined output
//! against the workload's `#@`/`verify:` rules.
//!
//! The bundled-example CI gate moved in-process
//! (`example_workloads_in_process.rs`), but `nbrs check` is a user
...
impl Sandbox {
    fn new() -> Self {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("nbrs-check-cli-{pid}-{nanos}"));
...
#[test]
fn nbrs_check_verifies_a_workload_directory() {
    let sb = Sandbox::new();
    ...
    // `nbrs check <dir>` walks the dir, re-spawns `nbrs run` per case
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_nbrs"))
        ...
        .current_dir(&sb.path)
        .output()
        .expect("spawn nbrs check");

    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        out.status.success(),
        "`nbrs check` should pass.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    assert!(
...
#[test]
fn nbrs_check_reports_slowest_workloads_for_a_directory() {
...
#[test]
fn nbrs_check_reports_failure_on_a_violated_rule() {
    let sb = Sandbox::new();
    let wl_dir = sb.path.join("workloads");
    std::fs::create_dir_all(&wl_dir).unwrap();

    // Same workload, but an `#@ expect` that the output will NOT match —
    // `nbrs check` must exit non-zero (the CI-gate contract).
    std::fs::write(
    ...
    assert!(
        !out.status.success(),
        "`nbrs check` must exit non-zero when a rule is violated.\nstdout:\n{}",
        String::from_utf8_lossy(&out.stdout)
    );
}

#[test]
fn nbrs_check_unknown_name_suggests_buried_leaf_matches() {
    // A bare stem the operator half-remembers, with the real workload
    // buried three directory levels below cwd. `nbrs check` must fail to
    // resolve it and offer the deep leaf-segment match as a "did you mean"
    // suggestion (the shared `nbrs_workload::suggest` engine). A stem that
...
#[test]
fn nbrs_check_dedups_repo_example_against_its_catalog_twin() {
