...
impl TempDir {
    fn new() -> Self {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("nbrs-plot-align-{pid}-{nanos}"));
...
// `nbrs/tests/output_channel_harness.rs`. Re-enable by reworking onto a
...
async fn default_draw_renders_cleanly_on_a_tty() {
    ...
    // screen exactly once, at shutdown, in place. nbrs has raw mode on for
