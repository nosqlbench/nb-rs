...
//! `nbrs replay`'s `phase_outcome` renderer with the ✗
...
//!    message + cycle).
//! 2. `nbrs replay --plain` renders the row through the
...
/// Run `nbrs run workload=... <extra>` against a freshly-
...
fn nbrs_replay_errors_filter_keeps_only_failed_outcomes() {
    ...
    cmd.arg("--plain");
    cmd.arg("--errors");
    let out = cmd.output().expect("exec nbrs replay --errors");
...
fn nbrs_replay_json_dumps_structured_outcome() {
    ...
    cmd.arg(format!("--session={}", session_path.display()));
    cmd.arg("--json");
    let out = cmd.output().expect("exec nbrs replay --json");
    let stdout = String::from_utf8_lossy(&out.stdout).to_string();
    assert!(out.status.success());
