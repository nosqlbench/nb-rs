...
//!
//! Each trial runs the REAL `nbrs check workload=<file>` as a subprocess in
//! its own sandbox cwd. That is deliberately the same command the CI gate is
//! defined against ("gates exactly what `nbrs check workload=examples`
...
fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = <repo>/nbrs for this crate's test targets.
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("nbrs crate lives one level under the workspace root")
        .to_path_buf()
}
...
fn run_check(workload: PathBuf) -> Result<(), Failed> {
    ...
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_nbrs"))
        .arg("check")
        .arg(format!("workload={}", workload.display()))
        .current_dir(&sandbox)
        .output()
        .map_err(|e| format!("spawn nbrs check: {e}"))?;
    let _ = std::fs::remove_dir_all(&sandbox);
    if out.status.success() {
        return Ok(());
    }
    Err(format!(
        "nbrs check failed for {}:\n--- stdout ---\n{}\n--- stderr ---\n{}",
        workload.display(),
...
fn main() {
    ...
    let examples = std::env::var("NBRS_TEST_EXAMPLES_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| repo_root().join("examples"));

    let trials: Vec<Trial> = nbrs_workload::verify::collect_workload_files(&examples)
        .into_iter()
        .map(|file| {
            // Name by path relative to examples/, so `nextest run -E
            // 'test(optimizer/)'` selects a subtree the same way
            // NBRS_TEST_EXAMPLES_DIR used to.
            let name = file
