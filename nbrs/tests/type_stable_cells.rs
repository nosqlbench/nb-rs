...
fn write_workload(label: &str, body: &str) -> PathBuf {
    ...
    dir.push(format!("nbrs_typestable_{label}_{}.yaml", std::process::id()));
...
fn narrowing_write_into_u64_cell_fails_with_diagnostic() {
    let wl = write_workload("narrow", r#"
bindings: |
  shared measured := 1
phases:
  probe:
...
"#);
    ...
    assert!(ok, "matching + widening writes must succeed, stderr={stderr}");
    assert!(stdout.contains("m=900"),
...
"#);
    let (stdout, stderr, ok) = run(&wl, &[]);
    assert!(ok, "explicit trunc_u64 narrowing must succeed, stderr={stderr}");
    assert!(stdout.contains("m=900"),
...
"#);
    let (stdout, stderr, ok) = run(&wl, &[]);
    assert!(ok, "annotated f64 cell must accept the f64 write, stderr={stderr}");
    assert!(stdout.contains("m=900.5"),
...
fn mismatched_annotation_is_a_compile_error() {
    let wl = write_workload("annot_bad", r#"
bindings: |
  shared name: str := 42
phases:
  p:
...
"#);
    ...
    assert!(stderr.contains("doesn't match the annotated type")
        || stderr.contains("shared binding"),
